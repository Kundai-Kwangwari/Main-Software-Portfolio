...
"""
    ...
    Credits:
    -------
        The complete program was written by B.Sai Chitra
...
"""
    ...
    def _creatingItemsets(self):
        self._transactionsDB, self._fuzzyValuesDB, self._Database = [], [], []
    ...
    def startMine(self):
        ...
        for j in range(len(self._transactionsDB)):
            ...
            quantity_list = self._fuzzyValuesDB[j]
            self._dbLen += 1
            
            for i in range(0, len(item_list)):
                regions = _Regions(float(quantity_list[i]), 3)
                item = item_list[i]
                if item in self._mapItemsLowSum.keys():
                    item_total_low = self._mapItemsLowSum[item]
                    item_total_low += regions.low
        ...
        for j in range(len(self._transactionsDB)):
            ...
            for i in range(0, len(item_list)):
                ...
                if item in self._mapItemSum and self._mapItemSum[item] >= self._minSup:
            ...
            for i in range(len(revisedTransaction) - 1, -1, -1):
                ...
                #print("item i", pair.item)
                remainUtil = 0
                for j in range(len(revisedTransaction) - 1, i, -1): #modification possible
                    remainUtil += revisedTransaction[j].quantity
                    """print("item j", revisedTransaction[j].item, " value: ", revisedTransaction[j].quantity, "total: ", remainUtil)
                    if tid == 3:
                        exit(2)"""
                remainingUtility = remainUtil
                if mapItemsToFFLIST.get(pair.item) is not None:
                    FFListOfItem = mapItemsToFFLIST[pair.item]
                    #print("pair values", pair.item, pair.quantity)
                    element = _Element(tid, pair.quantity, remainingUtility) #what are we actually doing here?? remaining utility
